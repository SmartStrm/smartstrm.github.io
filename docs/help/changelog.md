...
### v0.5.0
- 驱动
  - **新增 移动云盘 驱动**：通过 139 邮箱 Cookie 登录，支持文件列表、删除、重命名、新建目录、移动等完整文件操作，支持下载直链与 STRM 302 重定向播放
  - **移动云盘支持 CAS 秒传**：支持 `.cas` 秒传文件还原，临时文件自动清理（含回收站），直链缓存优化
...
### v0.4.20
- 302代理
  - **嵌入调用外部播放器按钮**：适配 Jellyfin 12.0 ，修复播放链接失效的问题
- 插件
  - 提取视频封面：为 ffmpeg/ffprobe 请求添加 UA 请求头，避免被 CDN 拦截（如光鸭云盘）
- STRM生成
  - 修复插件修改内容后导致签名失效的问题
- 其他
  - 添加隐私政策与使用条款
...
### v0.4.19
- 驱动
  - **新增 光鸭云盘 驱动**：支持 OAuth 授权登录，支持文件列表、下载与点播直链；受平台接口限制，目前仅提供只读能力
  - OpenList：获取二进制数据时补充 UA 请求头，修复部分场景下载刮削文件失败的问题
- 插件
  - 提取视频封面：兼容光鸭云盘，支持提取网盘预览图
